...
# endif
	...
	*/
	...
	if (newfile_size == 0) {
		rrd_file->file_len = file_size;
	} else {
		rrd_file->file_len = newfile_size;
		//append_archive_buff already done that 
...
no_lseek_necessary:
	...
	if (rdwr & RRD_CREAT) {
		...
		for (i = 0; i < (newfile_size - 1) / sizeof buf; ++i) {
			if ((data_len = pwrite(r->arop.fd, buf, sizeof buf, offset)) == -1) {
				ret = -RRD_ERR_WRITE5;
				goto out_close;
			}
			offset += data_len;
		...
		if (pwrite(rrd_simple_file->fd, buf,
					(newfile_size - 1) % sizeof buf, offset) == -1) {
			ret = -RRD_ERR_WRITE5;
			goto out_close;
		}

		lseek(rrd_simple_file->fd, 0, SEEK_SET);
		offset = file_offset + BLOCKSIZE;
	}
#endif
#endif
	rrd_file->mm_off = file_offset - PAGE_START(file_offset);
	rrd_file->mm_len = rrd_file->mm_off + rrd_file->file_len;
	data = mmap(0, rrd_file->mm_len, 
			rrd_simple_file->mm_prot, rrd_simple_file->mm_flags,
			r->arop.fd, PAGE_START(file_offset));
	...
	rrd_simple_file->mm_start = data;
	rrd_simple_file->file_start = data + rrd_file->mm_off;
	data = rrd_simple_file->file_start;
#endif
	if (rdwr & RRD_CREAT)
		goto out_done;
#ifdef USE_MADVISE
...
#ifdef HAVE_MMAP
	if (data != MAP_FAILED)
		munmap(rrd_simple_file->mm_start, rrd_file->mm_len);
#endif
...
#ifdef HAVE_MMAP
	ret = msync(rrd_simple_file->mm_start, rrd_file->mm_len, MS_ASYNC);
	if (ret != 0){
		ret = -RRD_ERR_MSYNC;
		goto out;
	}
	ret = munmap(rrd_simple_file->mm_start, rrd_file->mm_len);
	if (ret != 0){
		ret = -RRD_ERR_MUNMAP;
		goto out;
	}
